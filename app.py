...
def train_ann_model():
    """
    Train nhanh trên MNIST nếu model upload lên cloud không load được.
    Cách này giúp tránh lỗi không tương thích train.keras / train.h5 giữa các phiên bản Keras.
    """
    with st.spinner("Đang chuẩn bị model AD Numeriq lần đầu, vui lòng chờ 1-2 phút..."):
        ...
        # Lưu lại để các lần sau load nhanh hơn
        try:
...
def load_model():
    """
    Ưu tiên load train.keras.
    Nếu lỗi do khác phiên bản Keras, app sẽ tự train lại model ANN.
    """
    if os.path.exists("train.keras"):
        try:
            return tf.keras.models.load_model("train.keras", compile=False)
        except Exception as e:
            st.warning("File train.keras không tương thích với môi trường deploy. App sẽ tự train lại model ANN.")

    if os.path.exists("train.h5"):
        try:
            return tf.keras.models.load_model("train.h5", compile=False)
        except Exception as e:
            st.warning("File train.h5 không tương thích với môi trường deploy. App sẽ tự train lại model ANN.")

    return train_ann_model()
