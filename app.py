...
# Nếu chưa có logo.png thì app vẫn chạy bằng emoji 🤖.
LOGO_PATH = "logo.png"
...
def load_model():
    try:
        return tf.keras.models.load_model("train.keras")
    except Exception:
        return tf.keras.models.load_model("train.h5")
...
# =========================
# COMPONENT: LOGO + TÊN APP
# =========================
def show_app_logo(title="AD Numeriq", image_width=48):
    # Giữ logo chỉ làm icon app/tab, không hiển thị trong giao diện chính
    st.markdown(f"## 🤖 {title}")
...
def home_page():
    ...
    with nav1:
        show_app_logo("AD Numeriq", image_width=52)
...
def scanner_page():
    ...
    with nav1:
        show_app_logo("AD Numeriq Scanner", image_width=52)
